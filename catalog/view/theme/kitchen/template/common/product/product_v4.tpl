...
?>
<div class="product-block product-v4">
  <div class="image">
     <!-- Sale lable -->
  ...
  <div class="product-meta">
      ...
      <?php } ?>       
      <div class="action">
        <button data-toggle="tooltip" data-placement="top" class="btn btn-outline-light" title="<?php echo $button_cart; ?>" type="button" onclick="cart.add('<?php echo $product['product_id']; ?>');"><i class="icon-cart"></i></button> 
        ...
        <i class="fa fa-refresh"></i></button> 
         ...
         <?php if( isset($categoryPzoom) && $categoryPzoom ) { $zimage = str_replace( "cache/","", preg_replace("#-\d+x\d+#", "",  $product['thumb'] ));  ?>
          ...
          <?php if ( isset($quickview) && $quickview ) { ?>
            <a class="quickview iframe-link btn btn btn-outline-light" href="<?php echo $ourl->link('themecontrol/product','product_id='.$product['product_id']);?>" data-toggle="tooltip" data-placement="top" title="<?php echo $objlang->get('quick_view'); ?>" ><i class="fa-fw fa fa-eye"></i></a>
          <?php } ?>     
      </div>                
